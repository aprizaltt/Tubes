...
@section('content')
    <div class="flex justify-center gap-6 w-full p-12">
        ...
        <div class=" bg-gradient-to-b-greenish rounded-lg flex flex-col gap-4 p-8 w-full">
            <table class="border bg-gradient-to-b-dark w-full">
                <h2 class="font-semibold mb-4 text-xl text-white">Bidang Kegiatan Organisasi dan Kepemimpinan</h2>
                <tr class="text-white">
                    <th>No</th>
                    <th>Kegiatan</th>
                    <th>No Sk</th>
                ...
                @if (count($data))
                    @foreach ($data as $key => $item)
                        <tr class="bg-white">
                            ...
                            <td>
                                <button class="edit-btn mb-0.5 p-1 w-full block" data-id="{{ $item->id }}">
